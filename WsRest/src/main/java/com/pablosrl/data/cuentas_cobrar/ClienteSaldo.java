package com.pablosrl.data.cuentas_cobrar;

import java.util.Date;

public class ClienteSaldo {

	private String codCliente;
    private String nombreCliente;
    private String saldoGs;
    private String limiteCredito;
    private String posibleCompra;
    private String chequesPendientes;
    private String saldoFavor;
    private String notasCredito;
    private String saldoTotal;
    private String mensaje;
    
    private String descPrecio;
    private String descPlan;
    private String descCondicionVenta;
    private String descConsignacion;
    private Date fechaUltCompra;
    private String montoUltCompra;
    private Date fechaVenciPlan;
    private String coordenadasMaps;

    
	public String getCodCliente() {
		return codCliente;
	}
	public void setCodCliente(String codCliente) {
		this.codCliente = codCliente;
	}
	public String getNombreCliente() {
		return nombreCliente;
	}
	public void setNombreCliente(String nombreCliente) {
		this.nombreCliente = nombreCliente;
	}
	public String getSaldoGs() {
		return saldoGs;
	}
	public void setSaldoGs(String saldoGs) {
		this.saldoGs = saldoGs;
	}
	public String getLimiteCredito() {
		return limiteCredito;
	}
	public void setLimiteCredito(String limiteCredito) {
		this.limiteCredito = limiteCredito;
	}
	public String getPosibleCompra() {
		return posibleCompra;
	}
	public void setPosibleCompra(String posibleCompra) {
		this.posibleCompra = posibleCompra;
	}
	public String getChequesPendientes() {
		return chequesPendientes;
	}
	public void setChequesPendientes(String chequesPendientes) {
		this.chequesPendientes = chequesPendientes;
	}
	public String getSaldoFavor() {
		return saldoFavor;
	}
	public void setSaldoFavor(String saldoFavor) {
		this.saldoFavor = saldoFavor;
	}
	public String getNotasCredito() {
		return notasCredito;
	}
	public void setNotasCredito(String notasCredito) {
		this.notasCredito = notasCredito;
	}
	public String getSaldoTotal() {
		return saldoTotal;
	}
	public void setSaldoTotal(String saldoTotal) {
		this.saldoTotal = saldoTotal;
	}
	public String getMensaje() {
		return mensaje;
	}
	public void setMensaje(String mensaje) {
		this.mensaje = mensaje;
	}
	public String getDescPrecio() {
		return descPrecio;
	}
	public void setDescPrecio(String descPrecio) {
		this.descPrecio = descPrecio;
	}
	public String getDescPlan() {
		return descPlan;
	}
	public void setDescPlan(String descPlan) {
		this.descPlan = descPlan;
	}
	public String getDescCondicionVenta() {
		return descCondicionVenta;
	}
	public void setDescCondicionVenta(String descCondicionVenta) {
		this.descCondicionVenta = descCondicionVenta;
	}
	public String getDescConsignacion() {
		return descConsignacion;
	}
	public void setDescConsignacion(String descConsignacion) {
		this.descConsignacion = descConsignacion;
	}
	public Date getFechaUltCompra() {
		return fechaUltCompra;
	}
	public void setFechaUltCompra(Date fechaUltCompra) {
		this.fechaUltCompra = fechaUltCompra;
	}
	public String getMontoUltCompra() {
		return montoUltCompra;
	}
	public void setMontoUltCompra(String montoUltCompra) {
		this.montoUltCompra = montoUltCompra;
	}
    
    
    public Date getFechaVenciPlan() {
		return fechaVenciPlan;
	}
	public void setFechaVenciPlan(Date fechaVenciPlan) {
		this.fechaVenciPlan = fechaVenciPlan;
	}
	public String getCoordenadasMaps() {
        return coordenadasMaps;
    }

    public void setCoordenadasMaps(String coordenadasMaps) {
        this.coordenadasMaps = coordenadasMaps;
    }
    

    

}
